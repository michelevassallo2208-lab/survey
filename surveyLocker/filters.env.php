...
if (Session::UserHasOneOfProfilesIn('1,2,6')) {
	...
	$filter['fields']['tlId']->sourceCursor = Database::ExecuteSelect("SELECT id, CONCAT(surname, ' ', name) AS _label FROM users WHERE status='active' AND users.location ='battipaglia'  AND id IN (SELECT userId FROM pivot_users_profiles WHERE (profileId = '2')) ORDER BY _label");
	...
	$filter['sheet']->addField($filter['fields']['tlId']);
		...
		$filter['fields']['firstQ']->options = 'Si;No';
		$filter['fields']['firstQ']->usesNoSelection = true;
		$filter['sheet']->addField($filter['fields']['firstQ']);
		
		$filter['fields']['secondQ'] = new TextField('secondQ');
		$filter['fields']['secondQ']->label = 'Domanda 2';
		$filter['sheet']->addField($filter['fields']['secondQ']);
		

		$filter['fields']['thirdQ'] = new DropDownField('thirdQ');
		$filter['fields']['thirdQ']->label = 'Domanda 3';
		$filter['fields']['thirdQ']->options = 'Piano Terra (SF);Piano Terra (PM);Primo piano Wind3';
		$filter['fields']['thirdQ']->usesNoSelection = true;
		$filter['sheet']->addField($filter['fields']['thirdQ']);
